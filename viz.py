...
<section class="wide"><h2>Latent transition alignment</h2><p class="explain" id="cosine-note">Latent SmolVLA uses the frozen visual encoder, a transition predictor, and a linear action decoder. Cosine compares predicted and observed visual-token changes: 1 is aligned, 0 is unrelated, and -1 is opposite.</p><svg id="cosine" viewBox="0 0 1180 390"></svg></section>
<section class="wide"><h2>How the latent changes decoded actions</h2><p class="explain">Each graph replaces the predicted latent in one specific way and measures how much the decoded action chunk changes. Both latent variants remain visible.</p><div class="plots" id="controls"></div></section>
...
function renderCosine(){const svg=document.querySelector("#cosine"),d=DATA.transitions[0];if(!d)return;const values=d.cosine_by_step_and_view.flat(),mean=values.reduce((a,b)=>a+b,0)/values.length;document.querySelector("#cosine-note").textContent=`Latent SmolVLA uses the frozen visual encoder, a transition predictor, and a linear action decoder. Cosine 1 is aligned, 0 is unrelated, and -1 is opposite. The recorded mean is ${mean.toFixed(5)}, so the predicted direction is not aligned.`;const {X,Y}=plotFrame(svg,{x:[0,49],y:[-1,1],xTicks:[0,10,20,30,40,49],yTicks:[-1,-.5,0,.5,1],xLabel:"chunk step",yLabel:"cosine"});for(let camera=0;camera<2;camera++){const rows=d.cosine_by_step_and_view.map((v,i)=>({step:i,value:v[camera]})),color=camera?ORANGE:BLUE;svg.append(node("path",{d:rows.map((r,i)=>(i?"L":"M")+X(r.step)+","+Y(r.value)).join(" "),class:"line",stroke:color,"stroke-dasharray":camera?"6 4":""}));rows.forEach(r=>point(svg,X(r.step),Y(r.value),color,`<b>${d.name}, camera ${camera+1}</b><br>step ${r.step}<br>cosine ${r.value.toFixed(5)}`,2.5))}}
...
function render(){renderLegend();renderTraining();renderActionError();renderFinetuneCost();renderActions();renderCosine();renderControls()}const r=DATA.rollouts;document.querySelector("#readout").innerHTML=`<span>target rollout success<b>${r.successes}/${r.episodes}</b></span><span>evaluated variants<b>${r.methods}</b></span><span>training runs<b>${DATA.training.length}</b></span>`;renderProxy();renderAugmentations();renderSampleControls();render();
</script>'''
