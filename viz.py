"""Build the self-contained SmolVLA experiment report."""
...
def load_data():
    ...
    summaries = sorted((runs / "a100_final").glob("*/summary.json"))
    final = json.loads(summaries[-1].read_text()) if summaries else None
...
<title>SmolVLA on LIBERO</title><style>
...
</style><header><h1>SmolVLA on LIBERO</h1><div class="readout" id="readout"></div><div class="legend" id="method-filter"></div></header><main>
<section class="wide" id="final-section"><h2>Final A100 cost curve</h2><p class="explain">Every panel uses the same scale. Points are 20 rollout episodes for each of two training seeds; lines show their mean. No run is hidden.</p><div class="plots" id="final-curves"></div></section>
...
function renderFinal(){const section=document.querySelector("#final-section"),root=document.querySelector("#final-curves"),d=DATA.final;if(!d){section.style.display="none";return}root.replaceChildren();const panels=[["All tasks",null],["Task 0",0],["Task 1",1],["Task 2",2]],methods=[["mix_seen","Mix seen",BLUE],["lora_r32","LoRA r=32",ORANGE]];for(const[title,task]of panels){const box=document.createElement("div"),heading=document.createElement("div"),svg=node("svg",{viewBox:"0 0 340 255"});heading.className="metric-title";heading.textContent=title;box.append(heading,svg);const{X,Y}=plotFrame(svg,{x:[0,25],y:[0,1],xTicks:[0,5,10,25],yTicks:[0,.25,.5,.75,1],xLabel:"demonstrations",yLabel:"success"});for(const[id,name,color]of methods){const values=task===null?d.mean_success[id]:d.task_mean_success[id][task],rows=[5,10,25].map(n=>({demos:n,success:values[n]}));svg.append(node("path",{d:rows.map((r,i)=>(i?"L":"M")+X(r.demos)+","+Y(r.success)).join(" "),class:"line",stroke:color}));rows.forEach(r=>point(svg,X(r.demos),Y(r.success),color,`<b>${name}, ${title}</b><br>${r.demos} demos<br>success ${r.success.toFixed(3)}`,5))}if(task===null){point(svg,X(0),Y(d.mean_success.zero_shot[0]),"#64748b","<b>Zero-shot</b><br>success 0.000",5);point(svg,X(5),Y(d.mean_success.lapo_video_only[5]),"#7c3aed","<b>Continuous LAPO</b><br>5 demos<br>success 0.000",5)}root.append(box)}}
...
function render(){renderLegend();renderFinal();renderTraining();renderActionError();renderFinetuneCost();renderActions();renderCosine();renderTiny();renderLapo();renderControls()}const r=DATA.rollouts,f=DATA.final;document.querySelector("#readout").innerHTML=`<span>final eval episodes<b>${f?f.matrix.eval_episodes:0}</b></span><span>screening success<b>${r.successes}/${r.episodes}</b></span><span>training runs<b>${DATA.training.length}</b></span>`;renderProxy();renderAugmentations();renderSampleControls();render();
</script>'''
