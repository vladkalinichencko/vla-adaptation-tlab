...
function render(){renderLegend();renderTraining();renderActionError();renderFinetuneCost();renderActions();renderCosine();renderTiny();renderControls()}const r=DATA.rollouts;document.querySelector("#readout").innerHTML=`<span>target rollout success<b>${r.successes}/${r.episodes}</b></span><span>evaluated variants<b>${r.methods}</b></span><span>training runs<b>${DATA.training.length}</b></span>`;renderProxy();renderAugmentations();renderSampleControls();render();
</script>'''
