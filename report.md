...
Я проверял, сколько демонстраций целевой задачи нужно SmolVLA после претренинга на `libero_90`. Претрен использует пять тысяч шагов на 450 равномерно выбранных эпизодах из 4500. Целевая часть — первые 5, 10 или 25 демонстраций каждой из первых трёх задач `libero_goal`. Каждая итоговая точка посчитана на двадцати роллаутах, двух сидах обучения и одном общем сиде оценки.
...
Самым неожиданным оказалось, что почти идеально сошедшийся модуль спокойно уживается с нулевым успехом. Объясняю я это тем, что внутренние метрики измеряют согласованность модулей между собой, а не их полезность для управления. Поэтому главным инструментом диагностики стало сравнение настоящего латента с нулевым: оно меряет ровно перенос в действия. От альтернативы, что декодер просто недоучен, это отделяет только отдельный долгий контроль, которого у меня нет.
